Gitian building
================
*Setup instructions for a Gitian build of Qtep Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Qtep
...
- [Building Qtep Core](#building-qtep-core)
...
Clone the git repositories for qtep and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/boxaltcoin/qtep --recursive
git clone https://github.com/boxaltcoin/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/qtep/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the qtep repository under 'Fetch and create inputs' to install sources which require
...
To build Qtep Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the qtep repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/qtep/.git/
...
For example:
```bash
URL=https://github.com/laanwj/qtep.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit qtep=${COMMIT} --url qtep=${URL} ../qtep/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit qtep=${COMMIT} --url qtep=${URL} ../qtep/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit qtep=${COMMIT} --url qtep=${URL} ../qtep/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-qtep:
./bin/gbuild --commit qtep=${COMMIT},cpp-eth-qtep=${ETHCOMMIT} --url qtep=${URL},cpp-eth-qtep=${ETHURL} ../qtep/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the qtep git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../qtep/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/boxaltcoin/qtep-detached-sigs.git

BTCPATH=/some/root/path/qtep
SIGPATH=/some/root/path/qtep-detached-sigs

./bin/gbuild --url qtep=${BTCPATH},signature=${SIGPATH} ../qtep/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/qtep-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/qtep-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/qtep-osx-build.assert
```
...
[boxaltcoin/gitian.sigs](https://github.com/boxaltcoin/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@qtep.org) and he will commit them.
